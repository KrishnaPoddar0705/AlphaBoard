...
serve(async (req) => {
    ...
    try {
        ...
        // Check if requester is org admin, team creator, or removing themselves
        const isRemovingSelf = userId === targetUserId
        ...
        if (memberError) {
            console.error('Error removing user from team:', memberError)
            return new Response(
                JSON.stringify({ error: 'Failed to remove member from team', details: memberError?.message }),
                { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
            )
        }
